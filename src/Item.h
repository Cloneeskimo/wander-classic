...
struct WeaponLevel
{
	//Constructor
	WeaponLevel() {};
	WeaponLevel(ItemSubType weapon, int level)
	{
		this->weapon = weapon;
		this->level = level;
	}
...
public:
	...
	//         If else   -> -1
	Item(ItemType type, ItemSubType subType, std::string name, int value = 1, int quantity = 1, int level = -1, int effect = -1);
	Item() {};
	void construct(ItemType type, ItemSubType subType, std::string name, int value = 1, int quantity = 1, int level = -1, int effect = -1);
